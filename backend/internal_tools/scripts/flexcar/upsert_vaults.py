...
def create_user(api_log_file_path, api_base_url, api_key, row):
    ...
    return call_endpoint(api_log_file_path, api_base_url, api_key, 'POST', 'users', None, headers={'x-idempotency-id': order_id})

def vault_data(api_log_file_path, api_base_url, api_key, fp_id, row):
    vault_data = vault_blob_for_seed_file_row(row)
    return call_endpoint(api_log_file_path, api_base_url, api_key, 'PATCH', f"users/{fp_id}/vault", vault_data)
...
def run(api_base_url, api_key, seed_file_path, start_row, num_rows_to_process, api_log_file_path, ids_file_path):
...
if __name__ == "__main__":
    ...
    parser.add_argument("--start-row", help="Which idx in the seed file to begin processing at", required=True)    
    ...
    parser.add_argument("--ids-file-path", help="Path to file to *append* log of (Order_ID, fp_id)", required=True)    
