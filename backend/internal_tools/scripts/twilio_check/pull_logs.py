...
def pull_and_analyze_logs(keyword, start_date, end_date, log_filename):
        ...
        if keyword.lower() in message.body.lower():
            ...
            if message.status in ['delivered', 'sent']:
                ...
                with open(log_filename, 'a') as log_file:
                    log_file.write(f"{message.date_sent},{message.sid},{message.to},{message.from_},{message.status},{message.error_code},{message.error_message}\n")
    ...
    print(f"Successfully delivered messages containing keyword '{keyword}': {keyword_successful_messages}")
    print(f"Failed messages containing keyword '{keyword}': {keyword_failed_messages}")
    print(f"Success rate containing keyword '{keyword}': {keyword_successful_messages / keyword_messages * 100:.2f}%")
