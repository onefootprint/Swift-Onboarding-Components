...
def main(input_file, dry_run, batch_size, api_base, custodian_key):
    ...
    assert len(rows) == len(
        set(
            (row["scoped_vault_id"], row["data_identifier"], row["created_seqno"])
            for row in rows
        )
    ), "Input file contains duplicate seqnos for the same SV/DI"

    assert len(rows) == len(
        set((row["scoped_vault_id"], row["data_identifier"]) for row in rows)
    ), "Input file contains duplicate DIs"
...
def count_expiration_dis(m):
    return len(
        set(
            [
                (fp_id, di)
                for fp_id, dis in m.items()
                for di in dis
                if di.endswith(".expiration")
            ]
        )
    )
...
def migrate_batch(batch, dry_run, api_base, custodian_key):
    ...
    assert (
        len(batch)
        == updated + would_update + skipped_old_seqno + skipped_already_correct
    )
