...
// For example, not all countries or carriers will return the exact SIM swap date, but Lookup SIM Swap will return it as the last_sim_swap_date field when it's available.
// To standardize all these different methods of conveying SIM swap information, we use the fields swapped_period and swapped_in_period.
// Both of these values together allow you to answer a question like, “Was the SIM for this phone number swapped in the last 24 hours?”
// This is done by configuring the swapped_period field to hold the trailing time period that you’re interested in knowing if a SIM has been swapped, for example 24 hours, and using the swapped_in_period field to indicate yes or no.
// Find more information about the field descriptions from the API docs.
#[derive(Debug, Clone, Eq, PartialEq, serde::Deserialize, serde::Serialize)]
...
// Get the line type of a phone number including mobile, landline, fixed VoIP, non-fixed VoIP, toll-free, and more.
#[derive(Debug, Clone, Eq, PartialEq, serde::Deserialize, serde::Serialize)]
