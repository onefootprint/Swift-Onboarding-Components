...
impl ClientCertificateKey {
    ...
    pub fn parse_cert_and_key(cert: &[u8], key: &[u8]) -> Result<ClientCertificateKey, VaultProxyError> {
        let cert_and_key = vec![cert, key].concat();
...
mod tests {
    ...
    async fn test_parse_identity() {
        let cert = include_str!("../../../../ditto/src/dummy_cert/client.crt");
        let key = include_str!("../../../../ditto/src/dummy_cert/client.key");
        let server_cert = include_str!("../../../../ditto/src/dummy_cert/server.crt");
