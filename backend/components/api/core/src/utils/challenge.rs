...
impl<C: Serialize + DeserializeOwned + std::fmt::Debug> Challenge<C> {
    pub fn seal(&self, key: &ScopedSealingKey) -> FpResult<ChallengeToken> {
        let vec = key.seal(self)?.0;
        Ok(ChallengeToken(Base64Data(vec).to_string()))
    }

    pub fn unseal_string(key: &ScopedSealingKey, sealed: String) -> FpResult<Self> {
        Self::unseal(key, &ChallengeToken(sealed))
    }

    pub fn unseal(key: &ScopedSealingKey, sealed: &ChallengeToken) -> FpResult<Self> {
