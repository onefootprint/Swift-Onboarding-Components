use super::assert_allowed_for_sources;
...
impl<Type> WriteableVw<Type> {
    ...
    pub fn patch_data(
        ...
        self.create_bos_if_needed(conn, &mut request, &request_source)?;
    ...
    fn create_bos_if_needed(
        &self,
        conn: &mut TxnPgConn,
        request: &mut FingerprintedDataRequest,
        request_source: &DataRequestSource,
    ) -> FpResult<()> {
        ...
        let mut kyced_bos = kyced_bos.into_iter();
        let primary_bo = kyced_bos.next();
        if let Some(p_bo) = primary_bo {
            // Update the primary BO's ownership stake, which is only ever provided in this vault data right
            // now.
        ...
        // Add data for each beneficial owner's DIs to the request. This is a little hacky - we can only do
        // this because we know that we don't need to fingerprint data
        request.data.extend(extra_data.into_iter());
