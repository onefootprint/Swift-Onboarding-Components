...
#[test_state]
async fn test_backfill_deactivate(state: &mut State) {
    // Mimics another really specific case we've seen in prod where:
    // - User starts onboarding onto tenant
    // - User aborts and gets confused, finds footprint live and onboards onto fp live
    // - User never goes back to finish their onboarding onto first tenant
    // In this particular case, the first tenant has some speculative data that is overshadowed
    // by prefill data. When we prefill that data in, we should deactivate the existing speculative data
    let ff_client = state.feature_flag_client.clone();
    let data = state
        .db_pool
        .db_transaction(move |conn| -> ApiResult<_> { Ok(create_test_data_deactivate(conn, ff_client)) })
        .await
        .unwrap();
    ...
    // Should have added data to both tenants
    let result = result.unwrap().into_iter().next().unwrap();
    assert_eq!(result.1.len(), 2);
    state
        .db_pool
        .db_transaction(move |conn| -> ApiResult<_> {
            ...
            assert_have_same_elements(
                ...
                vec![
                    DI::Id(IDK::Email),
                    DI::Id(IDK::PhoneNumber),
                    // These are kind of weirdly included... to obey current visibility rules, but
                    // not necessarily because it's correct
                    // https://onefootprint.slack.com/archives/C04RHCM8FU0/p1701239287187669?thread_ts=1701232057.655059&cid=C04RHCM8FU0
                    DI::Id(IDK::FirstName),
                    DI::Id(IDK::LastName),
                    DI::Id(IDK::Ssn4),
                ],
            ...
            // Check email at tenant1 is prefilled from tenant2
            let dl1 = new_vw1.get_lifetime(IDK::Email).unwrap();
            let dl2 = new_vw2.get_lifetime(IDK::Email).unwrap();
            assert_eq!(dl1.source, DataLifetimeSource::Prefill);
            assert_eq!(dl1.origin_id, Some(dl2.id.clone()));

            // Check that the email at tenant1 was deactivated and replaced with prefill data
            use db_schema::schema::data_lifetime;
            use diesel::prelude::*;
            data_lifetime::table
                .filter(data_lifetime::deactivated_seqno.eq(dl1.created_seqno))
                .filter(data_lifetime::kind.eq(DI::Id(IDK::Email)))
...
fn create_test_data_deactivate(conn: &mut TxnPgConn, ff_client: Arc<dyn FeatureFlagClient>) -> TestData {
    let tenant1 = db::tests::fixtures::tenant::create(conn);
    let tenant2 = db::tests::fixtures::tenant::create(conn);
    let pb1_opts = ObConfigurationOpts {
        // PB1 only collects ssn4, doesn't collect dob
    ...
    // They then make a workflow that is auto-authorized because it is the only one (for now)
    let wf1 = db::tests::fixtures::workflow::create(conn, ff_client.clone(), &su1.id, &pb1.id, None);
    let wf1 = Workflow::lock(conn, &wf1.id).unwrap();
    Workflow::update(wf1, conn, WorkflowUpdate::is_authorized()).unwrap();
    ...
    // Some fun things happen here: in prod today, since the email isn't portable, we'll re-add it
    // to tenant 2
    let wf2 = db::tests::fixtures::workflow::create(conn, ff_client, &su2.id, &pb2.id, None);
    let data = vec![
        (IDK::Email.into(), PiiString::new("test1@onefootprint.com".into())),
        (IDK::FirstName.into(), PiiString::new("Hayes".into())),
        (IDK::LastName.into(), PiiString::new("Valley".into())),
