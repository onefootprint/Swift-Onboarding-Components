...
pub type SameTenantDupeDetail<'a> = (Vec<DupeKind>, TenantVw<Any>, &'a [TenantScope], DecryptedData);

impl<'a> DbToApi<SameTenantDupeDetail<'a>> for api_wire_types::SameTenantDupe {
    fn from_db((dupe_kinds, vw, scopes, decrypted_data): SameTenantDupeDetail<'a>) -> Self {
        let status = entity::status_from_sv(&vw.scoped_vault);
        let data = entity::entity_attributes(&vw, scopes, decrypted_data)
            .into_iter()
