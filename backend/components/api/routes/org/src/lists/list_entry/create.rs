...
pub async fn create_list_entry(
    ...
    let decrypted_list_key = SealingKey::new(state.enclave_client.decrypt_to_pii_bytes(&list.e_data_key.into(), &tenant.e_private_key).await?.into_leak())?;
    ...
    let list_entry = state
        ...
        .db_query(move |conn| -> ApiResult<_> {
            ...
            Ok(ListEntry::create(conn, &list_id, actor.into(), &key.seal_bytes(data.leak().as_bytes())?.into())?)
        })
        .await?;

    // actually decrypt using the key as a sanity check here
    let decrypted_data = PiiBytes::new(decrypted_list_key.unseal_bytes(AeadSealedBytes(list_entry.e_data.clone().0))?).try_into()?;
