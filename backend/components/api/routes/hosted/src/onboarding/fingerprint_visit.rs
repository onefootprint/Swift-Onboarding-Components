...
use api_core::auth::user::{CheckedUserAuthContext, UserAuth};
...
pub async fn post(
    ...
    tokio::spawn(async move {
        let _ = post_inner(state, user_auth, telemetry_headers, request, span).await;
    });

    EmptyResponse::ok().json()
}

#[tracing::instrument(skip_all, follows_from=[cause], err)]
async fn post_inner(
    state: web::Data<State>,
