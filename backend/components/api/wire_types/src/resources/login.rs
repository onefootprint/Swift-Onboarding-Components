...
pub struct OrgLoginResponse {
    ...
    /// If the email is associated with one tenant, a long-lived TenantRbAuth token to perform actions on the dashboard.
    /// If the email is associated with multiple tenants, a a short-lived WorkOsAuth token that can be used to select the tenant as which to auth.
