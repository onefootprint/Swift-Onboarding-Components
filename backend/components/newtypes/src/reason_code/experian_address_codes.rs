...
experian_reason_code_enum! {
    ...
    pub enum ExperianAddressAndNameMatchReasonCodes {
        ...
        #[ser = "A1"]
        #[footprint_reason_codes = ExpAddressRCH::new(FullName(Exact), FullAddress(Exact)).into()]
        A1,
        ...
        #[ser = "A2"]
        #[footprint_reason_codes = ExpAddressRCH::new(FullName(Exact), AddressExactExcept(vec![(StreetName, NoMatch)])).into()]
        A2,
        // Exact match on first and last name; Street number missing on input (all other fields match)
        #[ser = "A5"]
        #[footprint_reason_codes = ExpAddressRCH::new(FullName(Exact), AddressExactExcept(vec![(StreetNumber, NoMatch)])).into()]
        A5,
        // Exact match on first and last name; No match to city (all other fields match)
        #[ser = "A7"]
        #[footprint_reason_codes = ExpAddressRCH::new(FullName(Exact), AddressExactExcept(vec![(City, NoMatch)])).into()]
        A7,
        // Exact match on first and last name; No match to ZIP code (all other fields match)
        #[ser = "A8"]
        #[footprint_reason_codes = ExpAddressRCH::new(FullName(Exact), AddressExactExcept(vec![(Zip, NoMatch)])).into()]
        A8,
        // Exact match on first and last name; Partial match to street name (all other fields match)
        #[ser = "AC"]
        #[footprint_reason_codes = ExpAddressRCH::new(FullName(Exact), AddressExactExcept(vec![(StreetName, Partial)])).into()]
        AC,
        // Exact match on first and last name; Partial match to street name and no match to city (all other fields match)
        #[ser = "AF"]
        #[footprint_reason_codes = ExpAddressRCH::new(FullName(Exact), AddressExactExcept(vec![(StreetName, Partial), (City, NoMatch)])).into()]
        AF,
        ...
        #[ser = "AH"]
        #[footprint_reason_codes = ExpAddressRCH::new(FullName(Exact), AddressExactExcept(vec![(StreetName, Partial), (State, NoMatch)])).into()]
        AH,
        // Exact match on first and last name; Close match to street name; no match to ZIP code (all other fields match)
        #[ser = "AJ"]
        #[footprint_reason_codes = ExpAddressRCH::new(FullName(Exact), AddressExactExcept(vec![(StreetName, Partial), (Zip, NoMatch)])).into()]
        AJ,
        ...
        #[ser = "AU"]
        #[footprint_reason_codes = ExpAddressRCH::new(FullName(Exact), AddressExactExcept(vec![(State, NoMatch)])).into()]
        AU,
        ...
        #[ser = "C1"]
        #[footprint_reason_codes = ExpAddressRCH::new(FullName(Partial), FullAddress(Exact)).into()]
        C1,
        // Misspelling of either first OR last name; Misspelling of street name (all other fields match)
        #[ser = "C2"]
        #[footprint_reason_codes = ExpAddressRCH::new(FullName(Partial), AddressExactExcept(vec![(StreetName, Partial)])).into()]
        C2,
        // Misspelling of either first OR last name; No match to city (all other fields match)
        #[ser = "C7"]
        #[footprint_reason_codes = ExpAddressRCH::new(FullName(Partial), AddressExactExcept(vec![(City, NoMatch)])).into()]
        C7,
        // Misspelling of either first OR last name; No match to ZIP code (all other fields match)
        #[ser = "C8"]
        #[footprint_reason_codes = ExpAddressRCH::new(FullName(Partial), AddressExactExcept(vec![(Zip, NoMatch)])).into()]
        C8,
        ...
        #[ser = "CU"]
        #[footprint_reason_codes = ExpAddressRCH::new(FullName(Partial), AddressExactExcept(vec![(State, NoMatch)])).into()]
        CU,
        ...
        #[ser = "I1"]
        #[footprint_reason_codes = ExpAddressRCH::new(FirstAndLast((Partial, Exact)), FullAddress(Exact)).into()]
        I1,
        // First name misspelled, last name matches one of hyphenated last names; Misspelling of street name (all other fields match)
        #[ser = "I2"]
        #[footprint_reason_codes = ExpAddressRCH::new(FirstAndLast((Partial, Exact)), AddressExactExcept(vec![(StreetName, Partial)])).into()]
        I2,
        // First name misspelled, last name matches one of hyphenated last names; No match to city (all other fields match)
        #[ser = "I7"]
        #[footprint_reason_codes = ExpAddressRCH::new(FirstAndLast((Partial, Exact)), AddressExactExcept(vec![(City, NoMatch)])).into()]
        I7,
        // First name misspelled, last name matches one of hyphenated last names; No match to ZIP code (all other fields match)
        #[ser = "I8"]
        #[footprint_reason_codes = ExpAddressRCH::new(FirstAndLast((Partial, Exact)), AddressExactExcept(vec![(Zip, NoMatch)])).into()]
        I8,
        ...
        #[ser = "IU"]
        #[footprint_reason_codes = ExpAddressRCH::new(FirstAndLast((Partial, Exact)), AddressExactExcept(vec![(State, NoMatch)])).into()]
        IU,
        ...
        // Partial match on first name, exact match on last name; No match to state (all other fields
        #[ser = "XO"]
        #[footprint_reason_codes = ExpAddressRCH::new(FullName(NoMatch), FullAddress(NoMatch)).into()]
        XO,
