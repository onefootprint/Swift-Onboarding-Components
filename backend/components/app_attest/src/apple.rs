...
use x509_parser::{der_parser, nom};
...
impl AppleAppAttestationVerifier {
    ...
    pub fn new_default_ca<S>(allowed_app_bundle_ids: Vec<S>) -> Result<Self, AttestationError>
    where
        S: Into<String>,
    {
        Self::new(allowed_app_bundle_ids, vec![APPLE_APP_ATTESTATION_ROOT_CA_PEM])
    }

    /// create a new verifier with customizable CAs
    pub fn new<S>(allowed_app_bundle_ids: Vec<S>, root_ca_list: Vec<&[u8]>) -> Result<Self, AttestationError>
    where
        S: Into<String>,
    {
