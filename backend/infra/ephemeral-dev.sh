...
HC_API_KEY=$(pulumi config --stack footprint/dev-${name} get --path constants.honeycomb.ephemeralApiKey)
pulumi config --stack footprint/dev-${name} set --secret --path constants.honeycomb.apiKey ${HC_API_KEY}