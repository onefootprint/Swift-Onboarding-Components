...
def test_stepup_sandbox_outcome_kyb(kyb_sandbox_ob_config, sandbox_tenant):
    # Add a rule for BO
    ...
    obc1 = get(
        f"org/onboarding_configs/{kyb_sandbox_ob_config.id}",
        None,
        *sandbox_tenant.db_auths,
    )
    update_rules(
    ...
    # Create a new user
    user = BifrostClient.new_user(kyb_sandbox_ob_config)

    # Check that the sandbox outcome selector is correct
    body = get(
        "/hosted/onboarding/config",
        None,
        user.auth_token,
