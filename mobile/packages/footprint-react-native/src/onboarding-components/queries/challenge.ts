...
export const createChallenge = async (payload: CreateChallengeRequestPayload, options: RequestOptions) => {
  ...
  if (!identifyResponse.user) {
    return await signupChallenge(payload, options);
  }
  ...
  if (hasPhone) {
    return await loginChallenge({ kind: 'sms' }, { token: identifyResponse.user.token });
  }
  if (hasEmail) {
    return await loginChallenge({ kind: 'email' }, { token: identifyResponse.user.token });
  }
  throw new InlineOtpNotSupported('Cannot verify inline');
};
