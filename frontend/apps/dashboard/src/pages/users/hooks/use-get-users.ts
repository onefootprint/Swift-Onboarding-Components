...
import useUserFilters, { getCursors } from './use-users-filters';
...
const useGetUsers = (pageSize: number) => {
  ...
  const { filters, setFilter, setCursors } = useUserFilters();
  const getScopedUsers = useGetScopedUsers(pageSize, {
    onSuccess: ({ meta }) => {
      const count = meta?.count ?? 0;
      setTotalNumResults(count);
    },
  });
  ...
  const loadNextPage = () => {
    if (getScopedUsers.data?.meta.next) {
