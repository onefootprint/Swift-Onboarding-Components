...
const useValidateListEntries = () => {
  ...
  const validate = (kind: ListKind, entries: string) => {
    ...
    if (kind === ListKind.emailAddress) {
      return values.every(val => isEmail(val))
        ? undefined
        : t('email-address-invalid');
    }
    if (kind === ListKind.emailDomain) {
      return values.every(isEmailDomain)
        ? undefined
        : t('email-domain-invalid');
    }
    if (kind === ListKind.ssn9) {
      return values.every(isSSN9) ? undefined : t('ssn9-invalid');
    }
    if (kind === ListKind.phoneNumber) {
      return values.every(val => isPhoneNumber(val))
        ? undefined
        : t('phone-number-invalid');
    }
    if (kind === ListKind.phoneCountryCode) {
      return values.every(isPhoneCountryCode)
        ? undefined
        : t('phone-country-code-invalid');
    }
    if (kind === ListKind.ipAddress) {
      return values.every(val => isIpAddress(val))
        ? undefined
        : t('ip-address-invalid');
    }
