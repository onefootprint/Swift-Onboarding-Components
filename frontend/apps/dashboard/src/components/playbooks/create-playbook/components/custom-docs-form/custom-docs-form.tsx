...
type CustomDocsFormProps = {
  index?: number;
  formName:
    | UseFieldArrayProps<AdditionalDocsFormData, 'docs.custom'>['name']
    | UseFieldArrayProps<BusinessFormData, 'docs.custom'>['name'];
...
const CustomDocsForm = ({ index = 0, formName, onDelete, onCancel, onSubmit }: CustomDocsFormProps) => {
  ...
  const { t } = useTranslation('playbooks', { keyPrefix: 'create.custom-docs.form' });
  const id = useId();
  const { formState, register, trigger, setValue } = useFormContext<AdditionalDocsFormData>();
  const nameAttrs = {
