...
describe('ValidateCitizenships', () => {
  it('should reject an empty input when the legal status is Green Card or Visa', () => {
    ...
    expect(validateCitizenships('', UsLegalStatus.permanentResident)).toEqual(
      requiredError,
    );
  ...
  it('should reject any input when the legal status is not Green Card or Visa', () => {
    ...
    expect(validateCitizenships('ca, mx', EMPTY_SELECT_VALUE)).toEqual(
      shouldBeEmptyError,
    );
    expect(validateCitizenships('be, zw, cd', UsLegalStatus.citizen)).toEqual(
      shouldBeEmptyError,
    );
  });
  ...
  it('should reject any inputs that include the US', () => {
    ...
    expect(validateCitizenships('us', UsLegalStatus.permanentResident)).toEqual(
      usError,
    );
    expect(validateCitizenships('us, ca, mx', UsLegalStatus.visa)).toEqual(
      usError,
    );
  });
  ...
  it('should reject any inputs with invalid countries', () => {
    expect(
      validateCitizenships('asfd', UsLegalStatus.permanentResident),
    ).toEqual({
      errorType: CitizenshipsValidationError.INVALID,
      data: 'asfd',
    });
    expect(
      validateCitizenships(
        'asfd, 1234, canada',
        UsLegalStatus.permanentResident,
      ),
    ).toEqual({
      errorType: CitizenshipsValidationError.INVALID,
    ...
    expect(
      validateCitizenships('asfd, canada, ca,', UsLegalStatus.visa),
    ).toEqual({
      errorType: CitizenshipsValidationError.INVALID,
      data: 'asfd, canada',
    });
  ...
  it('should accept inputs with all valid countries (case insensitive) with the correct legal status', () => {
    expect(validateCitizenships('fr', UsLegalStatus.permanentResident)).toBe(
      undefined,
    );
    expect(validateCitizenships('fr, zw', UsLegalStatus.visa)).toBe(undefined);
    expect(validateCitizenships('CA, Hk, BE, bG', UsLegalStatus.visa)).toBe(
      undefined,
    );
  });
});
