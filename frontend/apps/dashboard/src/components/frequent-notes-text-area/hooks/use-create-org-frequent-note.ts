...
const createOrgFrequentNote = async (
  data: CreateOrgFrequentNoteRequest,
...
const useCreateOrgFrequentNote = () => {
  ...
  return useMutation(
    (req: CreateOrgFrequentNoteRequest) =>
      createOrgFrequentNote(req, authHeaders),
    {
      onSuccess: (resp: CreateOrgFrequentNoteResponse) => {
        queryClient.invalidateQueries(
          getOrgFrequentNotesQueryKey(resp.kind, authHeaders),
        );
      },
    },
  );
