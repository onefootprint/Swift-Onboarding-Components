...
Footprint's Vault Proxy will attempt to substitute in-place any tokens found in the body of the request with the corresponding detokenized, plain-text values. A token expression is starts by `{{` and ends with `}}`.
...
| Format = `{{ <footprint_user_token>.<data_identifier>.<attribute> }}` |
| --------------------------------------------------------------------- |
...
| Header                       | Value                                            |
| ---------------------------- | ------------------------------------------------ |
| `x-fp-proxy-footprint-token` | The Footprint user token to infer request tokens |

In this case, the token can omit the `<footprint_user_token>` above.

| Format = `{{ <data_identifier>.<attribute> }}` |
| ---------------------------------------------- |
| `{{ id.last_name }}`                           |
| `{{ id.ssn9 }}`                                |
| `{{ id.dob }}`                                 |
