...
Configure basic and advanced proxy settings right from the Footprint Developer Dashboard on [Vault Proxy Settings tab](https://dashboard.onefootprint.com/developers?tab=proxy_configs).
You can also programmtically create a proxy configuration [using the Admin API](#configure-via-admin-api).
