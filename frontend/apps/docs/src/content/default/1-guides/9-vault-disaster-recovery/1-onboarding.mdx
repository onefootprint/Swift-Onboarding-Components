...
First install the CLI tool used for enrollment and decryption. On a Mac, run:
...
## 5. Create a IAM role for bucket management

Footprint needs read and write access to the bucket to manage the encrypted data. Create a IAM role to delegate access like the one in the Terrraform examlpe below, substituting your external ID.
...
You do not need to save any of the output, but make note of the recipient (starting with `age1yubikey` which you will paste into the next step. You can retrieve the recipient again by running `age-plugin-yubikey --list`.
