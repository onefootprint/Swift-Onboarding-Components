---
page: default
slug: /articles/guides/onboarding-components-kyb
category: Guides
...
First of all, make sure you have followed the [KYC guide](/articles//guides/onboarding-components-kyc). The KYB process should be similar to the KYC and you'll need to collect Business and Business Owner information. [Here](https://demo.onefootprint.com/kyb/onboarding-components) you can find a live demo.
...
const BusinessData = ({ onDone }: { onDone: () => void }) => {
  ...
  const handleSubmit = (data: Di) => {
    fp.vault(data, { onSuccess: onDone });
  };
...
const PersonalData = ({ onDone }: { onDone: () => void }) => {
  ...
  const handleSubmit = (data: Di) => {
    fp.vault(data, { onSuccess: () => fp.handoff({ onComplete: onDone }) });
  };
