...
<inline-alert variant="info">
  ...
  guide](/integrate/api-auth) to securely connect to the Footprint API from your
  backend.
</inline-alert>
...
| `encrypt(algorithm, public_key)`     | Asymmetrically encrypts the contents to a public-key, where algorithm is either `rsa_pkcs1v15` or `ecies_p256_x963_sha256_aes_gcm` and the corresponding public key is a HEX-encoded DER-formatted public key.                                                 |
