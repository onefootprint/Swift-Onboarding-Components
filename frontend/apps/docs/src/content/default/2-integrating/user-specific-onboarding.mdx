---
page: default
slug: /integrate/user-specific-sessions
category: Integrate
...
| `update_auth_methods` | Allows the user to update any/all of their login methods (phone, email, passkey) using the Footprint Auth component.                                    | Auth          |
...
If you'd like to limit which auth methods are allowed to be updated, you can pass the list of desired methods when creating the token. For example, if you'd only like the user to be allowed to update their phone number, you can create a token like so:
...
## Step 3: Pass auth token into Footprint SDKs

You can provide this auth token to the Footprint SDK. This will ask the user to authenticate and then will launch them into the flow specified by the token kind and/or playbook. Check out the specific documentation for the SDK you're using.

For example, if you are launching the user into a KYC onboarding flow using our React integration, you would pass the `authToken` as a prop into the `FootprintVerifyButton` component like so:
