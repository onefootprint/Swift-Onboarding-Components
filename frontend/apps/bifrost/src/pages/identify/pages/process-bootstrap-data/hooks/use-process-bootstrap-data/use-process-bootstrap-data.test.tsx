...
import useProcessBootstrapData from './use-process-bootstrap-data';
import {
  withIdentify,
  ...
  withLoginChallenge,
  withSignupChallenge,
} from './use-process-bootstrap-data.test.config';
...
describe('useProcessBootstrapData hook', () => {
  ...
  describe('when bootstrap data has phone', () => {
    ...
    describe('when user not found', () => {
      ...
      it('calls onSuccess with signup challenge data', async () => {
        const { onSuccess, onError } = renderUseProcessBootstrapData({
          phoneNumber: '+12342345234',
        });

        await waitFor(() => {
          expect(onSuccess).toBeCalledWith(false, {
            challengeKind: ChallengeKind.sms,
            challengeToken: 'challengeToken',
            retryDisabledUntil: testDate,
          });
