...
const isSpanish = <T extends { l10n?: L10n }>(p: T): boolean =>
  p?.l10n?.language === 'es';

export const isError = (x: unknown): x is Error => x instanceof Error;

export const isAuth = (p: Obj): p is FootprintAuthProps =>
  p.kind === FootprintComponentKind.Auth;

export const isVerify = (p: Obj): p is FootprintVerifyProps =>
  p.kind === FootprintComponentKind.Verify;

export const isUpdateLoginMethods = (
  p: Obj,
): p is FootprintUpdateLoginMethodsProps =>
...
export const getLabel = (p: Obj) => {
  if (!p.label && isAuth(p))
    return isSpanish(p)
      ? 'Autenticar con Footprint'
      : 'Authenticate with Footprint';

  if (!p.label && isVerify(p))
    return isSpanish(p) ? 'Verificar con Footprint' : 'Verify with Footprint';

  return '';
};

export const getVariant = (
  p: Pick<FootprintButtonProps, 'kind' | 'variant' | 'dialogVariant'>,
): 'modal' | 'drawer' => {
