...
const create = async () => {
  ...
  if (shouldCamelCaseKeys) {
    ...
    fs.copyFileSync(path.join(currentDir, 'request.ts'), path.resolve('src/client/request.ts'));
    fs.copyFileSync(path.join(currentDir, 'transform-data.ts'), path.resolve('src/client/transform-data.ts'));
  }
