...
const renderDesktopBackPhotoRetry = (context: MachineContext) =>
  renderPage(context, <DesktopBackPhotoRetry />, 'backImageRetryDesktop');
...
describe('<DesktopBackPhotoRetry />', () => {
  ...
  describe('Contains the correct doc type in the title', () => {
    ...
    it('Residence card', () => {
      ...
      const title = screen.getByText('Green card · Back side');
      expect(title).toBeInTheDocument();
    });

    it('work permit', () => {
      renderDesktopBackPhotoRetry(initialContextWorkPermit);
      const title = screen.getByText('EAD card · Back side');
      expect(title).toBeInTheDocument();
    });

    it('ID card', () => {
      renderDesktopBackPhotoRetry(initialContextIdCard);
      const title = screen.getByText('ID card · Back side');
      expect(title).toBeInTheDocument();
    });
  });
  ...
  it('Contains the correct error messages', () => {
    ...
    const error2 = screen.getByText(
      "It looks like you uploaded the wrong side of your document. Please flip your ID and upload the back side of your driver's license from United States of America.",
    );
