...
import { IdDI, UsLegalStatus, VisaKind, isCountryCode } from '@onefootprint/types';
...
const validateUserData = (
  ...
  const isEmailValid = (email: string) => isStringValid(email) && isEmail(email);
  ...
  const isPhoneValid = (phoneNumber: string) => {
    ...
    const matchesSandboxNumber =
      phoneUtils.isNumberMatch(phoneNumber, sandboxNumber) === PhoneNumberUtil.MatchType.EXACT_MATCH;
  ...
  const isVisaKindValid = (visaKind: string) =>
    isStringValid(visaKind) && Object.values(VisaKind).includes(visaKind as VisaKind);

  const isUsLegalStatusValid = (status: string) =>
    isStringValid(status) && Object.values(UsLegalStatus).includes(status as UsLegalStatus);
  ...
  const filledEntries = Object.entries(userData).filter(([key, value]) => key in ValidatorByField && !!value?.value);
  ...
  const validatedEntries = filledEntries.filter(([key, value]) => {
    if (key === IdDI.state) {
      ...
      return !!ValidatorByField[IdDI.state](value.value as string, country?.value as string);
    }
  ...
  if (invalidKeys.length) {
    Logger.warn(`Filtering out invalid bootstrapped user data for keys: ${invalidKeys.join(', ')}`, {
