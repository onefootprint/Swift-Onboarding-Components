...
// provided cdos
export const isDiMissing = (
  cdos: CollectedKycDataOption[],
  collectedData?: KycData,
) => {
  const requiredDis = cdos.flatMap(
    option => CollectedKycDataOptionToRequiredAttributes[option],
  );

  // Adjust requiredDis depending on value of legal status
  if (
    cdos.some(cdo => cdo === CollectedKycDataOption.usLegalStatus) &&
